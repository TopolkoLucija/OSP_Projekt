...
ggplot(mldata, aes(duration)) + 
  geom_histogram(bins = 100, position = "dodge") + facet_wrap(score~.) + labs(x = "Trajanje", y = "Broj filmova") + scale_x_continuous(limits = c(60, quantile(data$duration, 0.98, na.rm = TRUE)))
```

```{r}
ggplot(mldata, aes(num_critic_for_reviews)) + 
  geom_histogram(bins = 100, position = "dodge") + facet_wrap(score~.) + labs(x = "Broj negativnih recenzija", y = "Broj filmova") 
```
