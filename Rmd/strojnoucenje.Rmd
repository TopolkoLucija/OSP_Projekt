...
```{r}
mldata$score <- ifelse(mldata$imdb_score < 3.33, "bad", ifelse(mldata$imdb_score < 6.66, "medium", "good"))
mldata$score <- factor(mldata$score, levels = c("bad", "medium", "good"))
...
```{r}
bad <- mldata[mldata$score=="bad",]
medium <- mldata[mldata$score=="medium",]
good <- mldata[mldata$score=="good",]
```


```{r}
over <- rbind(good, bad)
oversample <- ovun.sample(score~., data = over, method = "both", N = 3962)$data
```

```{r}
mldata <- rbind(mldata[mldata$score=="medium",], oversample)
```
...
ggplot(mldata, aes(duration)) + 
  geom_histogram(bins = 100, position = "dodge") + facet_wrap(score~.)
```
